...
def server_in_client_spawn_new_connection(connectionSocket):
    ...
    if match1:
        ...
        if rfc_number_of_file_to_send in RFC_dict:
            ...
            #last modified
            file_last_modified = os.stat('./RFC2/rfc%s.txt'%(rfc_number_of_file_to_send)).st_mtime
            
            #content-length
            file_length = os.stat('./RFC2/rfc%s.txt'%(rfc_number_of_file_to_send)).st_size
            
            #data
            name_of_file_to_send = './RFC2/rfc%s.txt'%(rfc_number_of_file_to_send)
                ...
                #','.join(str(x) for x in foo)
                        ...
                        if match3:
                            if int(match3.group(2)) == 200:
                                ...
                                filename = './RFC2/rfc_temp%s.txt'%(user_pref_three)
                                temp_file_desc = open(filename,'w')
                                temp_file_desc.write(''.join(str(x) for x in total_data))
                                temp_file_desc.close()
                                os.system('tail -n +8 ./RFC2/rfc_temp%s.txt > ./RFC2/tempfile.txt'%(user_pref_three))
                                os.system('mv ./RFC2/tempfile.txt ./RFC2/rfc%s.txt'%(user_pref_three))
                                os.system('rm ./RFC2/rfc_temp%s.txt'%(user_pref_three))
